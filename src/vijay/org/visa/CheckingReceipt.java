 ...
 * @author MAC
 */
public class CheckingReceipt extends javax.swing.JInternalFrame {

    /**
     * Creates new form PassportDocument
     */
    public CheckingReceipt() {
        initComponents();
        setSize(Toolkit.getDefaultToolkit().getScreenSize());
    }
    public CheckingReceipt(int businessVisaId) {
    ...
    public void viewDocuments()
    {
        ...
        try
        {
            ...
            PreparedStatement ps=con.prepareStatement("select checkingReceipt from tbl_business_visa_checking_recepit where businessVisaId=?");
            ps.setInt(1, businessVisaId);
            ResultSet rs=ps.executeQuery();
            while(rs.next())
            {
                checkingReceiptByte=rs.getBytes(1);
                ByteArrayInputStream bisPassport = new ByteArrayInputStream(checkingReceiptByte);
                try 
                { 
    ...
    private void btnBrowseActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBrowseActionPerformed
        ...
        filecheckingReceipt=fc.getSelectedFile();
        pathcheckingReceipt=filecheckingReceipt.getAbsolutePath();
        try
        {
            fincheckingReceipt=new FileInputStream(filecheckingReceipt);
            lencheckingReceipt=(int)filecheckingReceipt.length();
            bfImage=ImageIO.read(filecheckingReceipt);
            resizedImage=resize(bfImage,800,555);
            imageIcon=new ImageIcon(resizedImage);
            lblPassport.setIcon(imageIcon);
            if(imageIcon==null)
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        ...
        else
        {
            try
            {
                connection c=new connection();
                Connection con=c.conn();
                PreparedStatement ps=con.prepareStatement("insert into tbl_business_visa_checking_recepit(businessVisaId,checkingReceipt) values(?,?)");
                ps.setInt(1,businessVisaId);
                ps.setBinaryStream(2,fincheckingReceipt,lencheckingReceipt);
    ...
    private void btnUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUpdateActionPerformed
        ...
        else
        {
            try
            {
                connection c=new connection();
                Connection con=c.conn();
                PreparedStatement ps=con.prepareStatement("update tbl_business_visa_checking_recepit set checkingReceipt=? where businessVisaId=?");
                ps.setBinaryStream(1,fincheckingReceipt,lencheckingReceipt);
    ...
    private void btnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPrintActionPerformed
        ...
        ReportView re=new ReportView(path.concat("\\lib\\Reports\\Anar\\BusinessVisa\\CheckingReceipt.jasper"),para);
        re.setVisible(true);
    ...
    byte[] checkingReceiptByte;
    BufferedImage bfImage,resizedImage;
    ImageIcon imageIcon;
    int businessVisaId,lencheckingReceipt;
    String pathcheckingReceipt;
    File filecheckingReceipt;
    FileInputStream fincheckingReceipt;
