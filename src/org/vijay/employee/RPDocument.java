 ...
 * @author MAC
 */
public class RPDocument extends javax.swing.JInternalFrame {
    ...
    public void viewDocuments()
    {
        ...
        try
        {
            ...
            PreparedStatement ps=con.prepareStatement("select rpDocument from tbl_rp_employee where empId=?");
            ps.setInt(1, empId);
            ResultSet rs=ps.executeQuery();
            while(rs.next())
            {
                RPByte=rs.getBytes(1);
                ByteArrayInputStream bisPassport = new ByteArrayInputStream(RPByte);
                try 
                { 
    ...
    private void btnBrowseActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBrowseActionPerformed
        ...
        fileRP=fc.getSelectedFile();
        pathRP=fileRP.getAbsolutePath();
        try
        {
            finRP=new FileInputStream(fileRP);
            lenRP=(int)fileRP.length();
            bfImage=ImageIO.read(fileRP);
            resizedImage=resize(bfImage,400,555);
            imageIcon=new ImageIcon(resizedImage);
            lblPassport.setIcon(imageIcon);
            if(imageIcon==null)
    ...
    private void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
        ...
        else
        {
            try
            {
                connection c=new connection();
                Connection con=c.conn();
                PreparedStatement ps=con.prepareStatement("insert into tbl_rp_employee(empId,rpDocument) values(?,?)");
                ps.setInt(1,empId);
                ps.setBinaryStream(2,finRP,lenRP);
    ...
    private void btnUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUpdateActionPerformed
        ...
        else
        {
            try
            {
                connection c=new connection();
                Connection con=c.conn();
                PreparedStatement ps=con.prepareStatement("update tbl_rp_employee set rpDocument=? where empId=?");
                ps.setBinaryStream(1,finRP,lenRP);
                ps.setInt(2,empId);
